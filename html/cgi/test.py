...
<section>
<h2>Working directory &amp; relative file access</h2>
<table>""")
try:
    with open("data.txt") as _f:
        _content = _f.read().strip()
    print("<tr><td>CWD</td><td>{0}</td></tr>".format(os.getcwd()))
    print("<tr><td>data.txt</td><td class='ok'>{0}</td></tr>".format(_content))
except Exception as _e:
    print("<tr><td>CWD</td><td>{0}</td></tr>".format(os.getcwd()))
    print("<tr><td>data.txt</td><td style='color:red'>ERROR: {0}</td></tr>".format(_e))
print("""</table>
</section>
